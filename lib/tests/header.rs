...
#[test]
fn decode_legacy_header_type0_server() {
...
#[test]
fn decode_legacy_header_type0_gpu() {
    let data = fs::read("tests/samples/legacy_type0_gpu.crashlog").unwrap();
    let header = Header::from_slice(&data).unwrap().unwrap();

    let socket_id = header.socket_id();
    assert_eq!(socket_id, 0);

    let die_id = header.die_id();
    assert_eq!(die_id, None);

    let node = Node::from(&header);
    assert_eq!(
        node.get_by_path("record_collection_completed")
            .unwrap()
            .kind,
        NodeType::Field { value: 1 }
    );
}
